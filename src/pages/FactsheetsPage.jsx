...
<>
<Navbar />
<section className='pbr-hero-banner text-center'><h1 className='fw-bolder mb-3'>Factsheets</h1><p style={{color:'rgba(224,231,255,0.90)',maxWidth:580,margin:'0 auto'}}>Download comprehensive cancer data reports and factsheets from the PBCCR registry.</p></section>
<section className='py-5 px-5 bg-light'><div className='row g-4'>{FACTS.map((f,i)=>(<div className='col-md-4' key={i}><div className='card h-100 border-0 shadow-sm rounded-4 overflow-hidden'><div className='p-4'><span className='badge rounded-pill mb-3' style={{background:'rgba(147,51,234,0.1)',color:'#7e22ce',fontSize:12}}>{f.yr}</span><h5 className='fw-bold mb-2'>{f.title}</h5><p className='text-secondary small mb-3' style={{lineHeight:1.6}}>{f.desc}</p><div className='d-flex flex-wrap gap-1 mb-4'>{f.tags.map(t=>(<span key={t} className='badge bg-light text-secondary border small'>{t}</span>))}</div><button className='btn btn-sm w-100 fw-semibold rounded-pill' style={{background:'linear-gradient(105deg,#611DA2 0%,#A038D0 100%)',color:'#fff'}}>Download PDF</button></div></div></div>))}</div></section>
<Footer />
</>
)
}